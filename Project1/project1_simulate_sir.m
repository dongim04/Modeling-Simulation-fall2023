function [S, I, R, W] = project1_simulate_sir(s_0, i_0, r_0, beta, gamma, alpha, num_steps)
...
for step = 1 : (num_steps - 1)
    [S(step+1), I(step+1), R(step+1)] = project1_action_sir(S(step), I(step), R(step), beta, gamma, alpha);
end
